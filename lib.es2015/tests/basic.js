...
describe("initial test", function () {
    ...
    it("should generate meaningful classnames using stylesheet", function () {
        reinit();
        var classes = stylesheet({
            warning: {
                color: 'red'
            },
            success: {
    ...
    it("style should ignore 'false' 'null' and 'undefined'", function () {
        reinit();
        style({ color: 'blue' }, false && { color: 'red' }, null, undefined, { backgroundColor: 'red' });
    ...
    it("should generate unique instances when typestyle() is called", function () {
        var ts1 = createTypeStyle({ textContent: '' });
        var ts2 = createTypeStyle({ textContent: '' });
